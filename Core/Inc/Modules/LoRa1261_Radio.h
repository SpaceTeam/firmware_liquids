#ifndef LORA1261_RADIO_H
#define LORA1261_RADIO_H
...
enum class LoraoldAddr : uint16_t
{
...
class LoRa1261_Radio
{
	public:
	LoRa1261_Radio(const STRHAL_SPI_Id_t &spiId, const STRHAL_SPI_Config_t &spiConf, const STRHAL_GPIO_t &dio1, const STRHAL_GPIO_t &dio3, const STRHAL_GPIO_t &busyPin);
		...
		bool readReg(const LoraoldAddr &address, uint8_t *reg, uint8_t n = 0);
		uint8_t getReg(const LoraoldAddr &address);
		uint8_t getStatus();
		uint8_t getErrors();
		bool writeReg(const LoraoldAddr &address, uint8_t reg, uint16_t delay = 0);
