...
{
	return CANCOM::init(COMMode::STANDARD_COM_MODE);
}

COMState CANCOM::init(COMMode mode)
{
	state = COMState::SBY;
	...
	if (STRHAL_TIM_Heartbeat_Subscribe(STRHAL_TIM_TIM7, CANCOM::heartbeat) != 0)
		return state = COMState::ERR;
	...
	if (mode == COMMode::STANDARD_COM_MODE)
	{
		...
		if (STRHAL_CAN_Subscribe(STRHAL_FDCAN1, STRHAL_FDCAN_RX0, mainFilter, 2, CANCOM::standardReceptor) != 2)
			return state = COMState::ERR;
	}
	else if (mode == COMMode::BRIDGE_COM_MODE)
	{
		...
		if (STRHAL_CAN_Subscribe(STRHAL_FDCAN1, STRHAL_FDCAN_RX0, mainFilter, 1, CANCOM::internalReceptor) != 2)
					return state = COMState::ERR;

		if (STRHAL_CAN_Subscribe(STRHAL_FDCAN2, STRHAL_FDCAN_RX0, mainFilter, 1, CANCOM::externalReceptor) != 2)
					return state = COMState::ERR;
	}
	return state;
}
...
void CANCOM::standardReceptor(uint32_t id, uint8_t *data, uint32_t n)
{
	Can_MessageId_t msgId =
	{ 0 };
	Can_MessageData_t msgData =
...
void CANCOM::bridgeReceptor(STRHAL_FDCAN_Id_t bus_id, uint32_t id, uint8_t *data, uint32_t n)
{
	Can_MessageId_t incoming_id;
		incoming_id.uint32 = id;

		if(incoming_id.info.node_id == cancom->nodeId)
			CANCOM::standardReceptor(id, data, n);
		else
			STRHAL_CAN_Send(bus_id, id, data, n);
}
void CANCOM::internalReceptor(uint32_t id, uint8_t *data, uint32_t n)
{
	bridgeReceptor(STRHAL_FDCAN2, id, data, n);
}
void CANCOM::externalReceptor(uint32_t id, uint8_t *data, uint32_t n)
{
	bridgeReceptor(STRHAL_FDCAN1, id, data, n);
}
void CANCOM::heartbeat()
{
