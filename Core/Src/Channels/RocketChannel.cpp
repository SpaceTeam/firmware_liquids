...
RocketChannel::RocketChannel(uint8_t id, ADCChannel *oxPressureChannel, ADCChannel *fuelPressureChannel, ADCChannel *chamberPressureChannel, ServoChannel *oxServoChannel, ServoChannel *fuelServoChannel, PyroChannel *igniter0Channel, PyroChannel *igniter1Channel, uint32_t refreshDivider)
...
ROCKET_STATE RocketChannel::holddown(uint64_t time) {
	...
	if(time - timeLastTransition > 200)
		return POWERED_ASCENT;
