...
LoRa1261_Radio::LoRa1261_Radio(const STRHAL_SPI_Id_t &spiId, const STRHAL_SPI_Config_t &spiConf, const STRHAL_GPIO_t &dio1, const STRHAL_GPIO_t &dio3, const STRHAL_GPIO_t &busyPin) :
		spiId(spiId), spiConf(spiConf), dio1(dio1), dio3(dio3), busyPin(busyPin)
{
}

int LoRa1261_Radio::init()
{
	...
	//waitForBusy();
	uint8_t txClampConfig = getReg(LoraoldAddr::TX_CLAMP_CONFIG);
	waitForBusy();
	ret &= writeReg(LoraoldAddr::TX_CLAMP_CONFIG, txClampConfig | 0x1E, 1);
...
int LoRa1261_Radio::reset()
{
	return 0;
}

bool LoRa1261_Radio::isBusy()
{
	return (STRHAL_GPIO_Read(&busyPin) == STRHAL_GPIO_VALUE_L) ? false : true;
}

void LoRa1261_Radio::waitForBusy()
{
	while (isBusy());
}

bool LoRa1261_Radio::sendBytes(uint8_t *buffer, uint8_t n)
{
...
bool LoRa1261_Radio::writeReg(const LoraoldAddr &address, uint8_t reg, uint16_t delay)
{
	uint8_t cmd[4];
...
bool LoRa1261_Radio::writeCommand(const LoraOpcode &opcode, uint8_t *parameter, uint8_t n, uint16_t delay)
{
	uint8_t cmd[16];
...
bool LoRa1261_Radio::readReg(const LoraoldAddr &address, uint8_t *reg, uint8_t n)
{
	uint8_t cmd[3];
	uint16_t addr = static_cast<uint16_t>(address);
...
uint8_t LoRa1261_Radio::getStatus()
{
	uint8_t cmd[2];
...
uint8_t LoRa1261_Radio::getErrors()
{
	uint8_t cmd[2];
...
uint8_t LoRa1261_Radio::getReg(const LoraoldAddr &address)
{
	uint8_t reg = 0;
...
bool LoRa1261_Radio::setRFFrequency()
{
...
bool LoRa1261_Radio::setPAConfig()
{
	uint8_t parameter[4] =
...
bool LoRa1261_Radio::SetTxParams()
{
	uint8_t parameter[2] =
...
bool LoRa1261_Radio::SetBufferBaseAddress()
{
	uint8_t parameter[2] =
...
bool LoRa1261_Radio::setModulationParams()
{
	uint8_t parameter[4] =
...
bool LoRa1261_Radio::setPacketParams()
{
	uint8_t parameter[6] =
...
bool LoRa1261_Radio::setDioIrqParams()
{
	uint8_t parameter[8] =
